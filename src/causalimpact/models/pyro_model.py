...
class BayesianRegressionModel(nn.Module):
    ...
    def model(self, x_data, y_data=None):
        beta = pyro.sample("beta", dist.Normal(torch.zeros(self.input_dim), torch.ones(self.input_dim)).to_event(1))
    ...
    def guide(self, x_data, y_data=None):
        ...
        beta_scale = pyro.param("beta_scale", torch.ones(self.input_dim), constraint=dist.constraints.positive)
...
class PyroModel(BaseModel):
    ...
    def __init__(self, data, pre_period, post_period, index_col, target_col, covariates, model_args):
        ...
        self.svi = SVI(self.model.model, self.model.guide, self.optimizer, loss=Trace_ELBO())
    ...
    def fit(self):
        ...
        covariates = torch.tensor(pre_data[self.covariates.columns].values, dtype=torch.float)
    ...
    def predict(self):
        ...
        """
        covariates = torch.tensor(self.data[self.covariates.columns].values, dtype=torch.float)
        predictive = Predictive(self.model.model, guide=self.model.guide, num_samples=1000)
