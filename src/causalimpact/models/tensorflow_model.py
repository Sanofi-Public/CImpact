...
class TensorFlowModel(BaseModel):
    ...
    def build_model(self):
        ...
        local_level = tfp.sts.LocalLinearTrend(observed_time_series=observed_time_series)
        ...
        model = tfp.sts.Sum([local_level, seasonal, regression], observed_time_series=observed_time_series)
        return model
    ...
    def fit(self):
        ...
        try:
            if fit_method == 'hmc':
                ...
                self.samples, elbo_loss_curve = self.fit_with_vi(pre_data[self.target_col])
            ...
            self.variational_posteriors = None  # Reset to ensure predict doesn't proceed with incomplete fitting
    ...
    def predict(self):
        ...
        if self.samples is not None:
            forecast_dist = tfp.sts.forecast(
                model=self.model,
                observed_time_series=self.data[self.target_col][:self.pre_period[1]+1],  # Use pre-period data
                parameter_samples=self.samples,
                num_steps_forecast=len(self.post_data)  # Adjusted to include only post period length
            )
        else:
            forecast_dist = tfp.sts.forecast(
                model=self.model,
                ...
                num_steps_forecast=len(self.data) - self.pre_period[1] - 1  # Adjusted to include only post period length
            )

        one_step_dist = tfp.sts.one_step_predictive(
            model=self.model,
            observed_time_series=self.pre_data[self.target_col],  # Only pre-intervention data
            parameter_samples=self.samples
        )
        ...
        pre_pred = np.squeeze(one_step_dist.sample(self.model_args.get('num_results', 100)).numpy())
    ...
    def initialize_model(self, observed_time_series): 
        ...
        """
        local_level = tfp.sts.LocalLevel(observed_time_series=observed_time_series, name='local_level')
        seasonal = tfp.sts.Seasonal(num_seasons=12, observed_time_series=observed_time_series, name='seasonal')
        ...
        regression = tfp.sts.SparseLinearRegression(design_matrix=design_matrix, name='regression')

        model = tfp.sts.Sum([local_level, seasonal, trend, regression], observed_time_series=observed_time_series)
        return model