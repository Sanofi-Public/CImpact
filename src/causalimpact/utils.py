...
def validate_data(data, pre_period, post_period):
    ...
    """
    if pd.to_datetime(pre_period[0]) not in data.index or pd.to_datetime(post_period[1]) not in data.index:
...
def calculate_posterior_probabilities(post_effects_samples):
    ...
    Parameters:
    - post_effects_samples (np.array): Samples from the posterior distribution of the absolute effects.
