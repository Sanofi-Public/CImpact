...
class CausalImpactAnalysis:
    ...
    95% CI                  [{np.mean(rel_effect)-1.96*np.std(rel_effect):.2f}%, {np.mean(rel_effect)+1.96*np.std(rel_effect):.2f}%]            [{(np.sum(abs_effect)-1.96*np.std(cum_effect))/np.sum(predicted_mean*1.):,.2f}, {(np.sum(abs_effect)+1.96*np.std(cum_effect))/np.sum(predicted_mean*1.):,.2f}]          
